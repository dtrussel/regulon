...
static ron_fault_t pid_apply_candidate(ron_pid_instance_t       *inst,
                                       const ron_pid_config_t   *candidate)
{
    ron_fault_t fault;
...
ron_fault_t ron_pid_reset(ron_pid_instance_t *inst)
{
    ...
    pid_reset_state(&inst->state);
    inst->state.status = (inst->state.mode == RON_MODE_MANUAL)
        ? RON_STATUS_MANUAL_MODE
        : RON_STATUS_OK;
                             ...
                             ron_float_t         manual_out)
{
    ...
    {
        inst->state.integral = ron_clamp(clamped_output,
                                         inst->config.I_min,
                                         inst->config.I_max);
    ...
    inst->state.status = (mode == RON_MODE_MANUAL)
        ? RON_STATUS_MANUAL_MODE
        : RON_STATUS_OK;
