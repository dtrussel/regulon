...
- `regulon-c/include/ron/ron_lqr.h`
...
monotonic-latch harness. Phase 12 LQR/LQG optimal control covers
`RON-FR-730` through `RON-FR-739` and `RON-FR-750` through `RON-FR-759`
with active unit tests for `RON-TC-LQR-001` through `RON-TC-LQR-009` and
`RON-TC-LQG-001` through `RON-TC-LQG-009`, plus the `RON-TC-LQR-010-FV`
and `RON-TC-LQG-010-FV` CBMC harnesses. This closes the C11 roadmap.
...
## Phase 12: LQR and LQG Optimal Control

Status: **COMPLETE**. Closure evidence is recorded in
`docs/plans/c/c11-phase-12-lqr-lqg.md` and `docs/plans/c/c11-rollout.md`.
The discrete-time MIMO LQR (`ron_lqr`) and LQG (`ron_lqg`) controllers are
implemented with a shared DARE solver (`ron_lqr_dare_solve`, iterative
value recursion per SADS DD-19), `RON-TC-LQR-001`–`009` and
`RON-TC-LQG-001`–`009` unit tests, and the `RON-TC-LQR-010-FV` /
`RON-TC-LQG-010-FV` CBMC harnesses (both VERIFICATION SUCCESSFUL). Both
modules are active in the default build (`RON_ENABLE_LQR`/
`RON_ENABLE_LQG` now default ON). This closes the C11 roadmap: every
module across Phases 0–12 is implemented, tested, and traceable.

Requirement scope:

- `RON-FR-730` through `RON-FR-739` (LQR)
- `RON-FR-750` through `RON-FR-759` (LQG)

SADS/IS scope:

- `ron_lqr`, `ron_lqg`
- Public headers: `ron_lqr.h`, `ron_lqg.h`
- Sources: `ron_lqr.c`, `ron_lqg.c`
- Private shared header: `ron_lqr_internal.h` (DARE solver, reused by LQG)

Feature scope:

- Discrete algebraic Riccati equation (DARE) solver via iterative value
  recursion (DD-19); pre-computed and DARE gain modes.
- LQR: three state-estimate sources (external / Luenberger / Kalman),
  optional per-input integral augmentation, per-input saturation and
  rate limiting, runtime gain update, DARE-solution accessor.
- LQG: dual-DARE initialisation (LQR gain and Kalman gain solved
  independently per the separation principle, DD-20), predict/update
  delegation to the embedded Kalman filter, combined control step with
  LQR-equivalent output limiting.

Dependencies:

- `ron_matrix` (internal, shared with Kalman/state-space/observer)
- `ron_kalman`, `ron_observer`, `ron_statespace` (all already implemented)

Verification focus:

- DARE convergence against analytically known gains (double-integrator
  reference system) and non-convergence / non-positive-definite fault
  paths.
- Three-source state-estimate dispatch and cross-source rejection.
- Separation principle: an LQG solved via DARE reproduces the same gain
  as a standalone LQR solved against the identical cost matrices.
- Per-input saturation and both rate-limit directions.
- Defensive null/uninitialised/non-finite paths; 100% statement and
  branch coverage on both `ron_lqr.c` and `ron_lqg.c`.

Exit criteria:

- LQR and LQG are independently usable (LQR alone, or LQG's combined
  estimator + control law).
- `RON_ENABLE_LQR`/`RON_ENABLE_LQG` default ON; full-library build
  includes both by default.
- CBMC harnesses for output-bounded / no-heap claims pass.
