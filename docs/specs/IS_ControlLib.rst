...
include the aggregate header instead of the individual module headers:

.. code-block:: c

   #include "ron/ron.h"

``ron/ron.h`` transitively includes every public module header in dependency
order, each guarded by the corresponding ``RON_HAVE_<MODULE>`` macro from the
CMake-generated ``ron/ron_modules.h``.  When the library is built with a subset
of modules (see the ``RON_ENABLE_<MODULE>`` options), ``ron/ron.h`` includes
only the headers whose implementations are present.  The aggregate header adds
no symbols of its own; including individual module headers directly remains the
recommended pattern for size-sensitive consumers.  ``ron/ron_modules.h`` is
generated into the build tree, installed alongside the other public headers, and
is distinct from the optional user-supplied ``ron_config.h`` platform override.
