...
ZTEST(regulon_module, test_pid_converges_on_target)
{
    ...
    for (int i = 0; i < 400; i++) {
        ...
        zassert_equal(ron_pid_step(&pid, 1.0F, y, 0.01F, &u, &status), RON_FAULT_NONE,
                      "step faulted at i=%d", i);
        y += 0.01F * (u - y);
    }

    zassert_within(y, 1.0F, TOL, "did not converge: y=%d milli", (int) (y * 1000.0F));
}
...
ZTEST(regulon_module, test_pid_output_respects_saturation)
{
    ...
    for (int i = 0; i < 100; i++) {
        ...
        zassert_equal(ron_pid_step(&pid, 1000.0F, 0.0F, 0.01F, &u, &status), RON_FAULT_NONE,
                      "step faulted");
...
ZTEST(regulon_module, test_pid_fault_latches_and_clears)
{
    ...
    zassert_not_equal(ron_pid_step(&pid, 1.0F, (ron_float_t) NAN, 0.01F, &u, &status),
                      RON_FAULT_NONE, "NaN input was accepted");

    /* ...and stay faulted on subsequent good input, rather than healing. */
    zassert_not_equal(ron_pid_step(&pid, 1.0F, 0.0F, 0.01F, &u, &status), RON_FAULT_NONE,
                      "fault did not latch");
    ...
    zassert_equal(ron_pid_step(&pid, 1.0F, 0.0F, 0.01F, &u, &status), RON_FAULT_NONE,
...
{
    ron_lp1_t filter;
    ron_lp1_config_t cfg = { .alpha = 0.1F };
    ron_float_t y        = 0.0F;
    ...
    for (int i = 0; i < 200; i++) {
        ...
        zassert_equal(ron_lp1_step(&filter, 1.0F, &y), RON_FAULT_NONE, "step faulted");
        zassert_true(y >= prev, "output decreased on a rising step");
        zassert_true(y <= 1.0F + TOL, "output overshot the input");
    }
    zassert_within(y, 1.0F, TOL, "did not settle on the input");
}
...
{
    ron_trap_t traj;
    ron_trap_config_t cfg = { .v_max = 1.0F, .a_max = 2.0F };
    bool finished         = false;
    ron_float_t pos = 0.0F, vel = 0.0F, acc = 0.0F;

    zassert_equal(ron_trap_init(&traj, &cfg, 0.0F), RON_FAULT_NONE, "init rejected");
    zassert_equal(ron_trap_set_target(&traj, 5.0F), RON_FAULT_NONE, "target rejected");

    for (int i = 0; (i < 2000) && !finished; i++) {
        zassert_equal(ron_trap_step(&traj, 0.01F, &pos, &vel, &acc, &finished), RON_FAULT_NONE,
                      "step faulted");
        /* The velocity limit is a promise, not a suggestion. */
        zassert_true((vel <= cfg.v_max + TOL) && (vel >= -(cfg.v_max + TOL)),
                     "velocity %d milli exceeded v_max", (int) (vel * 1000.0F));
    }

    zassert_true(finished, "profile never reported completion");
    zassert_within(pos, 5.0F, TOL, "stopped short of target: %d milli", (int) (pos * 1000.0F));
}
...
ZTEST(regulon_module, test_lqr_dare_solve_stabilises_on_target)
{
    ...
    ron_float_t x[RON_LQR_MAX_STATES] = { 1.0F, 0.0F };
    ...
    *cfg = (ron_lqr_config_t){ 0 };
    ...
    cfg->A[0][0] = 1.0F; cfg->A[0][1] = 0.1F;
    cfg->A[1][0] = 0.0F; cfg->A[1][1] = 1.0F;
    cfg->B[0][0] = 0.005F;
    cfg->B[1][0] = 0.1F;
    ...
    for (int i = 0; i < 500; i++) {
        ron_float_t r[RON_LQR_MAX_INPUTS] = { 0.0F };
        ron_float_t u[RON_LQR_MAX_INPUTS];
        ron_status_t status;
        ron_float_t x0 = x[0], x1 = x[1];

        zassert_equal(ron_lqr_step(lqr, r, 0.1F, u, &status), RON_FAULT_NONE, "step faulted");

        x[0] = (cfg->A[0][0] * x0) + (cfg->A[0][1] * x1) + (cfg->B[0][0] * u[0]);
        x[1] = (cfg->A[1][0] * x0) + (cfg->A[1][1] * x1) + (cfg->B[1][0] * u[0]);
    }

    zassert_within(x[0], 0.0F, TOL, "position not regulated: %d milli", (int) (x[0] * 1000.0F));
    zassert_within(x[1], 0.0F, TOL, "velocity not regulated: %d milli", (int) (x[1] * 1000.0F));
}
#endif /* RON_HAVE_LQR */
